(function () {
  ...
  createApp({
    ...
    methods: {
      ...
      formatPieLabelPercent(percent) {
        const n = Number(percent || 0);
        if (!Number.isFinite(n)) return "0.0%";
        return `${n.toFixed(1)}%`;
      },
      getPieLabelConfig(data) {
        const list = Array.isArray(data) ? data : [];
        const dataLength = list.length;
        const maxNameLength = list.reduce((m, item) => {
          const l = String((item && item.name) || "").length;
          return l > m ? l : m;
        }, 0);
        const chartWidth = this.pieChart ? this.pieChart.getWidth() : 0;
        const baseFontSize = chartWidth > 640 ? 13 : chartWidth > 420 ? 12 : 11;
        const densityPenalty = dataLength >= 14 ? 2 : dataLength >= 10 ? 1 : 0;
        const namePenalty = maxNameLength >= 12 ? 2 : maxNameLength >= 8 ? 1 : 0;
        const fontSize = Math.max(10, baseFontSize - Math.max(densityPenalty, namePenalty));
        const lineHeight = fontSize + 4;
        const guideLength = dataLength >= 12 ? 10 : 14;
        const guideLength2 = maxNameLength >= 10 ? 18 : 12;
        const edgePadding = chartWidth > 0 ? Math.max(6, Math.floor(chartWidth * 0.02)) : 8;
        return {
          label: {
            show: true,
            position: "outside",
          ...
          labelLayout: (params) => {
            ...
            if (!width || !params || !params.labelRect) {
              return { moveOverlap: "shiftY", hideOverlap: true };
            }
      ...
      getPieOption(data) {
        ...
        const labelConfig = this.getPieLabelConfig(seriesData);
        return {
          tooltip: {
            trigger: "item",
            formatter: "{b}: {c} ({d}%)",
      ...
      async updatePieChart() {
        ...
        try {
          ...
          this.pieChart.setOption(this.getPieOption(finalData), true);
        } catch (e) {
