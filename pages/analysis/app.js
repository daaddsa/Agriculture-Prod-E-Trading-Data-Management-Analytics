(function() {
	...
	function parseUrlMarketId() {
		return Number(sessionStorage.getItem("ds-screen-market-id"));
	}
	...
	createApp({
		...
		methods: {
					...
					.then(function(list) {
						if (Array.isArray(list) && list.length > 0) {
							self.markets = list.map(function(m) {
								return {
									value: Number(m.marketId),
									label: m.marketName
								};
							});
							// 若当前 market 值不在列表中，默认选第一个
							var ids = self.markets.map(function(m) {
								return m.value;
							});
			...
			_lastNDates: function(dateStr, n) {
				var res = [];
			...
			_alignByDates: function(sourceDates, sourceY, targetDates) {
				var map = {};
				(sourceDates || []).forEach(function(d, i) {
					map[String(d)] = sourceY ? sourceY[i] : null;
				});
				return (targetDates || []).map(function(d) {
					var v = map[String(d)];
					if (v === undefined || v === null || v === "") return null;
					return Number(v);
				});
			...
			selectMenu: function(key) {
				...
				this.$nextTick(function() {
					requestAnimationFrame(function() {
						...
						if (key === "reports") {
							if (!Array.isArray(self.reportTypes) || self.reportTypes
								.length === 0) {
								self.loadReportTypes().then(function() {
									self.fetchReports();
								}).catch(function() {
									self.fetchReports();
								});
			...
			_refreshCurrentChart: function() {
				...
				if (m === "reports") this.fetchReports();
			},
			...
			initChart: async function() {
				...
				var aData = [];
				try {
					var res = await AnalysisAPI.getCountTrend(this.market, 1, this.date, {
						countDay: 30
					});
					var d = (res && res.data) || {};
				...
				var hasData = aData.length > 0 && aData.some(function(v) {
					...
					tooltip: {
						...
						formatter: function(params) {
							...
							var title = list.length > 0 ?
								(list[0].axisValueLabel != null ? String(list[0]
									.axisValueLabel) : String(list[0].axisValue)) :
								"";
							...
							list.forEach(function(p) {
								...
								lines.push(marker + name + ": " + fmt(p ? p.data :
									null) + " " + unit);
							});
			...
			initAmountChart: async function() {
				...
				var amtData = [];
				try {
					var res = await AnalysisAPI.getCountTrend(this.market, 2, this.date, {
						countDay: 30
					});
					var d = (res && res.data) || {};
					var targetDates = this._lastNDates(this.date, 30);
				...
				var hasData = amtData.length > 0 && amtData.some(function(v) {
					return Number.isFinite(v);
				});
				chart.setOption({
					tooltip: {
			...
			initAvgPriceChart: async function() {
				...
				try {
					...
					var results = await Promise.all([
						AnalysisAPI.getCountTrend(this.market, 3, this.date, {
							countDay: 30
						})
						.catch(function(e) {
							console.warn("交易均价(countType=3)请求失败:", e);
							return null;
						}),
						AnalysisAPI.getCountTrend(this.market, 4, this.date, {
							countDay: 30
						})
						.catch(function(e) {
							console.warn("交易均价剔除异常(countType=4)请求失败:", e);
							return null;
						}),
					]);
					...
					avgCleanData = this._alignByDates(avgCleanD.xList || [], avgCleanD.yList || [],
						targetDates);
				} catch (e) {
					console.error("交易均价走势加载失败:", e);
				}

				var hasData = (avgData.length > 0 && avgData.some(function(v) {
						return Number.isFinite(v);
					})) ||
					(avgCleanData.length > 0 && avgCleanData.some(function(v) {
						return Number.isFinite(v);
					}));
				chart.setOption({
					tooltip: {
						...
						formatter: function(params) {
							...
							list.forEach(function(p) {
								var marker = p && p.marker ? p.marker : "";
								var name = p && p.seriesName ? p.seriesName : "";
								lines.push(marker + name + ": " + fmt2(p ? p.data :
									null) + " " + unit);
							});
							return lines.join("<br/>");
						},
			...
			initCommissionChart: async function() {
				...
				try {
					...
					var results = await Promise.all([
						AnalysisAPI.getCommissionFeeLvTrend(this.market, this.date, {
							countDay: 30
						}),
						AnalysisAPI.getCountTrend(this.market, 5, this.date, {
							countDay: 30
						}),
					]);
					...
					(function() {
						var f = (rateData || []).filter(function(v) {
							return Number.isFinite(v);
						});
						if (f.length > 0) {
							var maxAbs = Math.max.apply(null, f.map(function(v) {
								return Math.abs(v);
							}));
							if (maxAbs <= 0.001) {
								rateData = rateData.map(function(v) {
									return Number.isFinite(v) ? v * 10000 : v;
								});
							} else if (maxAbs <= 1) {
								rateData = rateData.map(function(v) {
									return Number.isFinite(v) ? v * 1000 : v;
								});
							}
						}
					})();
				...
				var hasData = (rateData.length > 0 && rateData.some(function(v) {
						return Number.isFinite(v);
					})) ||
					(feeData.length > 0 && feeData.some(function(v) {
						return Number.isFinite(v);
					}));
				...
				chart.setOption({
					tooltip: {
						...
						formatter: function(params) {
							...
							var lines = [title];
							list.forEach(function(p) {
								var unit = p && p.seriesName === "交易佣金费率" ? "‰" :
									"万元";
								var marker = p && p.marker ? p.marker : "";
								var name = p && p.seriesName ? p.seriesName : "";
								lines.push(marker + name + ": " + fmt2(p ? p.data :
			...
			initProvinceChart: async function() {
				...
				var marketCleanData = [];
				try {
					var res = await AnalysisAPI.getPriceTrend(this.market, this.province16, this.date, {
						countDay: 30
					});
					var d = (res && res.data) || {};
					this.insights = d.interpret;
					...
					(function() {
						...
						for (var i = len - 1; i >= 0; i--) {
							var a = factoryData[i];
							var b = marketCleanData[i];
							if (a != null && b != null && Number.isFinite(a) && Number.isFinite(
								b)) {
								end = i;
								break;
							}
				...
				var hasData = (factoryData.some(function(v) {
						return v != null;
					})) ||
					(marketCleanData.some(function(v) {
						return v != null;
					}));
				...
				try {
					...
					var mapped = Array.isArray(list) ?
						list.map(function(it) {
							return {
								label: String(it.dictLabel || ""),
								value: String(it.dictValue || "")
							};
						}) :
						[];
					// 在前面加上"全部"选项
			...
			fetchReports: async function() {
				...
				try {
					if (rt === "" && (!Array.isArray(this.reportTypes) || this.reportTypes.length ===
						0)) {
						await this.loadReportTypes();
					}
					if (rt !== "") {
						// ---- 具体类型：服务端分页 ----
						var res = await AnalysisAPI.getCustomReports(this.market, pageNum, pageSize, rt,
							title);
